...
class Reader():
    ...
    def get_file_reader(self, fileName):
        '''Выбор нужного Reader в зависимости от файла, на вход передается путь к файлу'''
        try:
...
class RsaReader():
    ...
    def read_file(self):
        ...
        try:
            ...
            for line in f:
        #            если в файле найдена строка с KKS выделить KKS и заполнить detector_list KKS и пустым списком со значениями    
        ...
        #           удалить первое число у первого датчика(у первого датчика 4 значение, первое из которых относится ко всей строке)
...
class SVBUReader():
    '''Реализация класса для считывания инфомации из СВБУ файлов'''
    def __init__(self,fileName):
        super().__init__()
        self.fileName = fileName
...
class SVBUFixedReader():
    '''Реализация класса для считывания инфомации из СВБУ файлов с фиксированным шагом'''
    def __init__(self,fileName):
        super().__init__()
        self.fileName = fileName
